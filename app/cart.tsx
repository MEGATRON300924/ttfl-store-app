    ...
    <View style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {items.length === 0 ? (
          ...
          <>
            {items.map((item) => (
              <GlassCard key={item.productId + ":" + (item.variationKey ?? "default")} style={styles.itemCard} intensity={24}>
                <View style={styles.itemRow}>
                  ...
                  <View style={styles.itemInfo}><Text style={styles.itemName} numberOfLines={2}>{item.name}</Text><Text style={styles.itemPrice}>{item.currency}{item.price.toLocaleString()}</Text>{item.variationLabel ? <Text style={styles.variation}>{item.variationLabel}</Text> : null}<Text style={styles.stock}>{item.stock} available</Text></View>
                  <Pressable onPress={() => removeItem(item.productId, item.variationKey)} hitSlop={8}><Ionicons name="trash-outline" size={19} color="#999" /></Pressable>
                </View>
                <View style={styles.itemBottom}><View style={styles.quantity}><Pressable onPress={() => updateQuantity(item.productId, item.quantity - 1, item.variationKey)} style={styles.quantityButton}><Ionicons name="remove" size={17} color="#111" /></Pressable><Text style={styles.quantityText}>{item.quantity}</Text><Pressable onPress={() => updateQuantity(item.productId, item.quantity + 1, item.variationKey)} style={styles.quantityButton}><Ionicons name="add" size={17} color="#111" /></Pressable></View><Text style={styles.lineTotal}>{item.currency}{(item.price * item.quantity).toLocaleString()}</Text></View>
              </GlassCard>
            ))}
